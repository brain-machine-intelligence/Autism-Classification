...
save_dir = "chckPts/"
save_prefix = "save"
summaryFolderName = "Original_random_summary/"
...
if len(sys.argv) == 2:
    ...
    images = np.load('image_norm_0.25.npy')
...
img_size = mri_height
minRadius = 10  # zooms -> minRadius * 2**<depth_level>
...
def get_next_input(output, glimpse_step):
    ...
    mean_loc = tf.random_normal((batch_size, LOC_DIM), mean = 0, stddev= 0.6)
...
def model():
    ...
    initial_loc = tf.random_normal((batch_size, LOC_DIM), mean = 0, stddev= 0.6)
...
with tf.device('/device:GPU:0'):
    with tf.Graph().as_default():
        ...
        for exp in range(cross_validation_times):
            ...
            for epoch in range(start_step + 1, max_iters):
                ...
                if epoch % 20 == 0:
                    ...
                    if epoch % test_step == 0:
                        ...
                        save_path = saver.save(sess, "Original_random_Experiment_" + str(exp) + '_' + save_dir + save_prefix + str(epoch) + ".ckpt")
            ...
            sess.close()
            path_cv = 'Data/Original_Cross_Validation/random/'

            if os.path.isdir(path_cv):
                pass
            else:
                os.mkdir(path_cv)
            np.save(path_cv + 'random_test_acc_experiment_' + str(exp), mean_acc)
            np.save(path_cv + 'random_test_err_experiment_' + str(exp), std_acc)