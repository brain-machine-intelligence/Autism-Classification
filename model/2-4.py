...
h_conv1 = tf.layers.conv3d(h_trans_ext, 16, 3, strides=1, padding='same', activation=tf.nn.relu)
h_conv1 = tf.layers.max_pooling3d(h_conv1, 2, 2)
h_conv2 = tf.layers.conv3d(h_conv1, 16, 3, strides=1, padding='same', activation=tf.nn.relu)
h_conv2 = tf.layers.max_pooling3d(h_conv2, 2, 2)
...
iter_per_epoch = 1000
n_epochs = 200 # Feel free to increase the epochs for better results
...
tranlated_img, h_fc_loc2_mat_value = sess.run([h_trans_ext, h_fc_loc2_mat], feed_dict={x: batch_xs, y: batch_ys, keep_prob: 1.0})

print(h_fc_loc2_mat_value)
np.save('3C_3D_original_mri_images', np.reshape(batch_xs_orig, (-1,  mri_height, mri_width, mri_channel)))
np.save('3C_3D_translated_mri_images', tranlated_img)
