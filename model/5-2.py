...
save_dir = "chckPts/"
save_prefix = "save"
summaryFolderName = "Original_noloc_summary/"
...
if len(sys.argv) == 2:
    ...
    no_hot_labels = np.load('labels_data_no_hot.npy')
...
channels = 170               # mnist are grayscale images
img_size = 256
minRadius = 20  # zooms -> minRadius * 2**<depth_level>
...
def get_glimpse(loc, glimpse_step):
    ...
    act_glimpse_hidden = tf.layers.dropout(act_glimpse_hidden, rate = 0.5, training= tf_is_training)
    ...
    act_loc_hidden = tf.layers.dropout(act_loc_hidden, rate= 0.5, training = tf_is_training)            ###dropout
    ...
    glimpseFeature = tf.layers.dropout(glimpseFeature, rate=0.5,  training = tf_is_training)           ###dropout
    # return g
...
def model():
    ...
    for t in range(nGlimpses):
        ...
        hiddenState = tf.layers.dropout(hiddenState, rate=0.5,  training = tf_is_training)
...
def evaluate():
    ...
    feed_dict = {inputs_placeholder: nextX, labels_placeholder: nextY,\
                     ...
                     tf_is_training : False, loc_sd : var_loc}
...
with tf.device('/device:GPU:0'):
    with tf.Graph().as_default():
        ...
        inputs_placeholder = tf.placeholder(tf.float32, shape=(batch_size, channels, img_size, img_size), name="images")
        ...
        for exp in range(cross_validation_times):
            full_index = np.arange(82)
            test_index = np.random.choice(82, 10, replace= False)
            ...
            min_loc_imgsize = depth * minRadius / (2.0 * img_size)
            max_loc_imgsize = 1 - depth * minRadius / (2.0 * img_size)

            min_loc_channel = depth * minRadius / (2.0 * channels)
            max_loc_channel = 1 - depth * minRadius / (2.0 * channels)
            ...
            for i in range(depth):
                ...
                mean_left_corner = mean_one_12 - (i + 1) * minRadius / (2.0 * img_size)
                # mean_left_corner = tf.clip_by_value(mean_left_corner, 0, 1)
                mean_right_corner = mean_one_12 + (i + 1) * minRadius / (2.0 * img_size)
                ...
                sample_left_corner = sample_one_12 - (i + 1) * minRadius / (2.0 * img_size)
                # sample_left_corner = tf.clip_by_value(sample_left_corner, 0, 1)
                sample_right_corner = sample_one_12 + (i + 1) * minRadius / (2.0 * img_size)
            ...
            for epoch in range(start_step + 1, max_iters):
                ...
                nextX = images_clip[batch_samples]
                ...
                feed_dict = {inputs_placeholder: nextX, labels_placeholder: nextY,
                                    onehot_labels_placeholder: dense_to_one_hot(nextY),
                                    tf_is_training: True, loc_sd : var_loc}
                ...
                fetches = [train_op, J2, J21, J22, J_total, adjusted_locs, d_crops, outputs,
                           ...
                           avg_b, rminusb, mean_locs, sampled_locs, lr, loc_sd, mean_one_0, sample_one_0]
                ...
                rminusb_fetched, mean_locs_fetched, sampled_locs_fetched, lr_fetched, loc_fetched, \
                ...
                if epoch % 20 == 0:
                    ...
                    var_loc *= 0.999
                    # var_loc *= 0.9998
                    var_loc = np.maximum(var_loc, 0.01)
                    ...
                    print(('Exp: %d Step %d: cost = %.3f (J_total = [%.3f, %.3f, %.3f, %.3f]) reward = %.3f (%.3f sec) label_zero = %.3f b = %.5f, LR = %.5f, Loc_SD = %.5f '
                               % (exp, epoch, cost_fetched, J_total_value[0], J_total_value[1],J_total_value[2],J_total_value[3], reward_fetched, duration, label_zero, avg_b_fetched, lr_fetched, loc_fetched)))
                    ...
                    if epoch % test_step == 0:
                        ...
                        save_path = saver.save(sess, "Original_noloc_Experiment_" + str(exp) + '_' + save_dir + save_prefix + str(epoch) + ".ckpt")
            ...
            sess.close()
            path_cv = 'Data/Original_Cross_Validation/'

            if os.path.isdir(path_cv):
                pass
            else:
                os.mkdir(path_cv)
            np.save(path_cv + 'noloc_test_acc_experiment_' + str(exp), mean_acc)
            np.save(path_cv + 'noloc_test_err_experiment_' + str(exp), std_acc)